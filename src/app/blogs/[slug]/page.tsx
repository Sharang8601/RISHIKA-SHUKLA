...
export default function BlogDetailPage() {
  ...
  useEffect(() => {
    // 1. Journal Book-Opening Page Transition
    const tl = gsap.timeline();

    tl.to(".book-center-text", {
      opacity: 0,
      y: -20,
      duration: 0.5,
      ease: "power2.out",
    ...
    tl.to(".book-left-page", {
      xPercent: -100,
      duration: 1.4,
      ease: "power4.inOut"
    }, "-=0.2");

    tl.to(".book-right-page", {
      xPercent: 100,
      duration: 1.4,
      ease: "power4.inOut"
    }, "<");

    tl.set(".book-curtain", { display: "none" });

    // 2. Cover Elements Stagger (runs after curtain begins opening)
    tl.fromTo(
    ...
    tl.fromTo(
      ".article-sidebar-fade",
      { opacity: 0, y: 30 },
      { opacity: 1, y: 0, duration: 0.8, ease: "power3.out", stagger: 0.1 },
      "-=0.5"
    );

    // 3. Image Scroll Parallax
    gsap.fromTo(
      ".article-hero-img",
      { yPercent: -10 },
      {
        ...
        ease: "none",
        scrollTrigger: {
          trigger: ".article-hero-image-wrapper",
          start: "top bottom",
          end: "bottom top",
          scrub: true
        }
  ...
  return (
    <>
      {/* Journal Book-Opening Curtain Overlay */}
      <div className="book-curtain fixed inset-0 z-[100] flex pointer-events-none">
        <div className="book-left-page w-1/2 h-full bg-surface-container border-r border-outline-variant flex items-center justify-end pr-8 pointer-events-auto">
          <span className="book-center-text font-headline-md text-headline-md text-primary font-semibold block origin-right">
            Rishika
          </span>
        </div>
        <div className="book-right-page w-1/2 h-full bg-surface-container flex items-center justify-start pl-8 pointer-events-auto">
          <span className="book-center-text font-headline-md text-headline-md text-primary font-semibold block origin-left">
            Shukla
          </span>
        </div>
      </div>
      ...
      <main className="w-full px-margin-mobile md:px-gutter max-w-container-max mx-auto pt-stack-lg md:pt-[60px] pb-section-gap">
        <article className="mb-section-gap">
          ...
          <header className="mb-stack-lg md:mb-[60px] text-center md:text-left max-w-3xl mx-auto md:mx-0">
            <div className="article-category-badge flex items-center justify-center md:justify-start space-x-stack-sm mb-stack-md flex-wrap opacity-0">
            ...
            <h1 className="font-display-lg-mobile text-display-lg-mobile md:font-display-lg md:text-display-lg text-on-background mb-stack-lg leading-tight">
              <span className="block overflow-hidden relative pb-1">
                <span className="block article-title-reveal">The Architecture of Silence:</span>
              </span>
              <span className="block overflow-hidden relative pb-1">
                <span className="block article-title-reveal">Designing Whitespace in</span>
              </span>
              <span className="block overflow-hidden relative pb-1">
                <span className="block article-title-reveal text-primary">Digital Environments.</span>
              </span>
            </h1>

            {/* Mobile Author Info */}
            <div className="article-sidebar-fade flex items-center justify-center md:justify-start space-x-stack-sm md:hidden mb-stack-md border-y border-outline-variant py-stack-sm opacity-0">
              <img 
          ...
          <figure className="w-full mb-stack-lg md:mb-[80px]">
            <div className="article-hero-image-wrapper w-full h-[400px] md:h-[600px] overflow-hidden rounded-xl border border-surface-variant shadow-[0_4px_20px_rgba(0,0,0,0.04)] relative">
              <img 
                className="article-hero-img w-full h-[120%] object-cover absolute top-0 left-0" 
                alt="Minimalist room" 
                src={blogData.imgUrl}
              />
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-gutter relative">
            ...
            <aside className="article-sidebar-fade opacity-0 hidden lg:flex lg:col-span-1 flex-col items-center sticky top-[120px] h-fit space-y-stack-md pt-2">
            ...
            <div className="lg:col-span-7 xl:col-span-7 text-left">
              ...
              <h2 className="font-headline-md text-headline-md text-on-background mt-section-gap mb-stack-md font-semibold">
            ...
            <aside className="article-sidebar-fade opacity-0 hidden lg:block lg:col-span-4 xl:col-span-3 pl-4 sticky top-[120px] h-fit text-left">
              {/* Author Profile Card */}
